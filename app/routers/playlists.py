...
async def list_playlists(
    ...
    return await playlist_service.get_all_playlists(
        db_session=db_session, limit=limit, offset=offset, is_system=is_system, owner_id=str(user.id)
    )


@router.get("/{playlist_id}", response_model=PlaylistDetailOut)
async def get_playlist(playlist_id: str, db_session: DBSessionDep, user: CurrentUserDep):
...
async def create_playlist(payload: PlaylistCreate, db_session: DBSessionDep, user: CurrentUserDep):
...
async def update_playlist(
    playlist_id: str, payload: PlaylistUpdate, db_session: DBSessionDep, user: CurrentUserDep
):
    """Update a playlist's name or description."""
    return await playlist_service.update_playlist(
        playlist_id=playlist_id, payload=payload, db_session=db_session, owner_id=str(user.id)
    )


@router.delete("/{playlist_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_playlist(playlist_id: str, db_session: DBSessionDep, user: CurrentUserDep):
    """Delete a playlist."""
...
async def set_playlist_videos(
    playlist_id: str, payload: PlaylistSetVideos, db_session: DBSessionDep, user: CurrentUserDep
):
...
async def add_video_to_playlist(
    playlist_id: str, payload: PlaylistAddVideo, db_session: DBSessionDep, user: CurrentUserDep
):
...
async def bulk_add_videos(
    playlist_id: str, payload: PlaylistAddVideos, db_session: DBSessionDep, user: CurrentUserDep
):
...
async def move_video(
    playlist_id: str, payload: PlaylistMoveVideo, db_session: DBSessionDep, user: CurrentUserDep
):
...
async def set_position(
    playlist_id: str, payload: PlaylistSetPosition, db_session: DBSessionDep, user: CurrentUserDep
):
...
async def shuffle_playlist(playlist_id: str, db_session: DBSessionDep, user: CurrentUserDep):
...
async def clear_playlist_videos(playlist_id: str, db_session: DBSessionDep, user: CurrentUserDep):
...
async def remove_video_from_playlist(
    ...
    await playlist_service.remove_video_from_playlist(
        playlist_id=playlist_id, video_id=video_id, db_session=db_session, owner_id=str(user.id)
    )
