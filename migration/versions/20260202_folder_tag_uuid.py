...
def upgrade() -> None:
    ...
    # Association tables
    op.add_column('channel_tags', sa.Column('uuid_tag_id', sa.String(36), nullable=True))
...
def downgrade() -> None:
    ...
    op.add_column('folders', sa.Column('int_id', sa.Integer(), nullable=True, autoincrement=True))
    op.add_column('folders', sa.Column('int_parent_id', sa.Integer(), nullable=True))
    op.add_column('tags', sa.Column('int_id', sa.Integer(), nullable=True, autoincrement=True))
    ...
    with op.batch_alter_table('folders', schema=None) as batch_op:
        ...
        batch_op.create_primary_key('pk_folders', ['id'])
        batch_op.create_index('ix_folders_id', ['id'])
        batch_op.create_foreign_key('fk_folders_parent_id', 'folders', ['parent_id'], ['id'])
    ...
    with op.batch_alter_table('channels', schema=None) as batch_op:
        batch_op.create_foreign_key('fk_channels_folder_id', 'folders', ['folder_id'], ['id'])

    with op.batch_alter_table('channel_tags', schema=None) as batch_op:
        batch_op.create_primary_key('pk_channel_tags', ['channel_id', 'tag_id'])
        batch_op.create_foreign_key('fk_channel_tags_tag_id', 'tags', ['tag_id'], ['id'], ondelete='CASCADE')

    with op.batch_alter_table('video_tags', schema=None) as batch_op:
        batch_op.create_primary_key('pk_video_tags', ['video_id', 'tag_id'])
        batch_op.create_foreign_key('fk_video_tags_tag_id', 'tags', ['tag_id'], ['id'], ondelete='CASCADE')
